...
$python_executable -m pip install "$package_name==$package_ok_version" numpy==1.26.4 --prefer-binary -i https://mirrors.aliyun.com/pypi/simple/
...
for i in $current_dir/changelog-info/$package_name/*; do
    ...
    if [[ "$package_version" == "0."* ]]; then
        python_executable=~/ve5/bin/python
    elif [[ "$package_version" == "2."* ]]; then
        python_executable=~/ve9/bin/python
    elif [[ "$package_version" == "1."* ]]; then
        python_executable=~/ve8/bin/python
    else
        python_executable=~/ve9/bin/python
    fi
    pip_not_worked=""

    $python_executable -m pip uninstall --yes $package_name numpy python-dateutil
    $python_executable -m pip install "$package_name==$package_version" "numpy<2" --prefer-binary -i https://mirrors.aliyun.com/pypi/simple/ || pip_not_worked="1"
